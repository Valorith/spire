...
func (e *AccountController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "accounts/count", e.getAccountsCount, nil),
...
func (e *AccountController) updateAccount(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Account] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *AccountController) createAccount(c echo.Context) error {
	...
	db := e.db.Get(models.Account{}, c).Model(&models.Account{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Account{}, account)
		...
		// record event
		event := fmt.Sprintf("Created [Account] [%v] data [%v]", account.ID, strings.Join(fields, ", "))
...
func (e *AccountController) deleteAccount(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Account] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
func (e *AccountController) getAccountsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Account{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
