...
func (e *GridController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "grids/count", e.getGridsCount, nil),
...
func (e *GridController) createGrid(c echo.Context) error {
	...
	db := e.db.Get(models.Grid{}, c).Model(&models.Grid{})
...
// @Success 200 {array} models.Grid
// @Failure 500 {string} string "Bad query request"
// @Router /grids/count [get]
func (e *GridController) getGridsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Grid{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
