...
func (e *GraveyardController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "graveyards/count", e.getGraveyardsCount, nil),
...
func (e *GraveyardController) updateGraveyard(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Graveyard] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *GraveyardController) createGraveyard(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Graveyard{}, graveyard)
		...
		// record event
		event := fmt.Sprintf("Created [Graveyard] [%v] data [%v]", graveyard.ID, strings.Join(fields, ", "))
...
func (e *GraveyardController) deleteGraveyard(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Graveyard] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getGraveyardsCount godoc
// @Id getGraveyardsCount
// @Summary Counts Graveyards
// @Accept json
// @Produce json
// @Tags Graveyard
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Graveyard
// @Failure 500 {string} string "Bad query request"
// @Router /graveyards/count [get]
func (e *GraveyardController) getGraveyardsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Graveyard{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
