...
func (e *FriendController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "friends/count", e.getFriendsCount, nil),
...
func (e *FriendController) createFriend(c echo.Context) error {
	...
	db := e.db.Get(models.Friend{}, c).Model(&models.Friend{})
...
// getFriendsCount godoc
// @Id getFriendsCount
// @Summary Counts Friends
// @Accept json
// @Produce json
// @Tags Friend
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
func (e *FriendController) getFriendsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Friend{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
