...
func (e *LootdropController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "lootdrops/count", e.getLootdropsCount, nil),
...
func (e *LootdropController) updateLootdrop(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Lootdrop] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *LootdropController) createLootdrop(c echo.Context) error {
	...
	db := e.db.Get(models.Lootdrop{}, c).Model(&models.Lootdrop{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Lootdrop{}, lootdrop)
		...
		// record event
		event := fmt.Sprintf("Created [Lootdrop] [%v] data [%v]", lootdrop.ID, strings.Join(fields, ", "))
...
func (e *LootdropController) deleteLootdrop(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Lootdrop] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getLootdropsCount godoc
// @Id getLootdropsCount
// @Summary Counts Lootdrops
// @Accept json
// @Produce json
// @Tags Lootdrop
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Lootdrop
// @Failure 500 {string} string "Bad query request"
// @Router /lootdrops/count [get]
func (e *LootdropController) getLootdropsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Lootdrop{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
