...
func (e *CharacterCurrencyController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_currencies/count", e.getCharacterCurrenciesCount, nil),
...
func (e *CharacterCurrencyController) updateCharacterCurrency(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterCurrency] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterCurrencyController) createCharacterCurrency(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterCurrency{}, c).Model(&models.CharacterCurrency{})
	...
	err := db.Create(&characterCurrency).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterCurrency{}, characterCurrency)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterCurrency] [%v] data [%v]", characterCurrency.ID, strings.Join(fields, ", "))
...
func (e *CharacterCurrencyController) deleteCharacterCurrency(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterCurrency] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getCharacterCurrenciesCount godoc
// @Id getCharacterCurrenciesCount
// @Summary Counts CharacterCurrencies
// @Accept json
// @Produce json
// @Tags CharacterCurrency
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterCurrency
// @Failure 500 {string} string "Bad query request"
// @Router /character_currencies/count [get]
func (e *CharacterCurrencyController) getCharacterCurrenciesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterCurrency{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
