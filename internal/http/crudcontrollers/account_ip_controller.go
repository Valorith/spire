...
func (e *AccountIpController) updateAccountIp(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [AccountIp] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *AccountIpController) createAccountIp(c echo.Context) error {
	...
	db := e.db.Get(models.AccountIp{}, c).Model(&models.AccountIp{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.AccountIp{}, accountIp)
		...
		// record event
		event := fmt.Sprintf("Created [AccountIp] [%v] data [%v]", accountIp.Accid, strings.Join(fields, ", "))
...
func (e *AccountIpController) deleteAccountIp(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [AccountIp] [%v] keys [%v]", result.Accid, strings.Join(ids, ", "))
