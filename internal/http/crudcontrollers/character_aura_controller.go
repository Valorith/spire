...
func (e *CharacterAuraController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_auras/count", e.getCharacterAurasCount, nil),
...
func (e *CharacterAuraController) createCharacterAura(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterAura{}, c).Model(&models.CharacterAura{})
	...
	err := db.Create(&characterAura).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getCharacterAurasCount godoc
// @Id getCharacterAurasCount
// @Summary Counts CharacterAuras
// @Accept json
// @Produce json
// @Tags CharacterAura
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterAura
// @Failure 500 {string} string "Bad query request"
// @Router /character_auras/count [get]
func (e *CharacterAuraController) getCharacterAurasCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterAura{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
