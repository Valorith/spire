...
func (e *TitleController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "titles/count", e.getTitlesCount, nil),
...
func (e *TitleController) updateTitle(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Title] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *TitleController) createTitle(c echo.Context) error {
	...
	db := e.db.Get(models.Title{}, c).Model(&models.Title{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Title{}, title)
		...
		// record event
		event := fmt.Sprintf("Created [Title] [%v] data [%v]", title.ID, strings.Join(fields, ", "))
...
func (e *TitleController) deleteTitle(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Title] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getTitlesCount godoc
// @Id getTitlesCount
// @Summary Counts Titles
// @Accept json
// @Produce json
// @Tags Title
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Title
// @Failure 500 {string} string "Bad query request"
// @Router /titles/count [get]
func (e *TitleController) getTitlesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Title{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
