...
func (e *CompletedSharedTaskMemberController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "completed_shared_task_members/count", e.getCompletedSharedTaskMembersCount, nil),
...
func (e *CompletedSharedTaskMemberController) createCompletedSharedTaskMember(c echo.Context) error {
	...
	db := e.db.Get(models.CompletedSharedTaskMember{}, c).Model(&models.CompletedSharedTaskMember{})
	...
	err := db.Create(&completedSharedTaskMember).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getCompletedSharedTaskMembersCount godoc
// @Id getCompletedSharedTaskMembersCount
// @Summary Counts CompletedSharedTaskMembers
// @Accept json
// @Produce json
// @Tags CompletedSharedTaskMember
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CompletedSharedTaskMember
// @Failure 500 {string} string "Bad query request"
// @Router /completed_shared_task_members/count [get]
func (e *CompletedSharedTaskMemberController) getCompletedSharedTaskMembersCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CompletedSharedTaskMember{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
