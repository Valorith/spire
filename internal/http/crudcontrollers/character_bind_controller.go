...
func (e *CharacterBindController) updateCharacterBind(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterBind] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterBindController) createCharacterBind(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterBind{}, c).Model(&models.CharacterBind{})
	...
	err := db.Create(&characterBind).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterBind{}, characterBind)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterBind] [%v] data [%v]", characterBind.ID, strings.Join(fields, ", "))
...
func (e *CharacterBindController) deleteCharacterBind(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterBind] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
