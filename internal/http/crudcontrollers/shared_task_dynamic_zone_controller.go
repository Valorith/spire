...
func (e *SharedTaskDynamicZoneController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "shared_task_dynamic_zones/count", e.getSharedTaskDynamicZonesCount, nil),
...
// getSharedTaskDynamicZonesCount godoc
// @Id getSharedTaskDynamicZonesCount
// @Summary Counts SharedTaskDynamicZones
// @Accept json
// @Produce json
// @Tags SharedTaskDynamicZone
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.SharedTaskDynamicZone
// @Failure 500 {string} string "Bad query request"
// @Router /shared_task_dynamic_zones/count [get]
func (e *SharedTaskDynamicZoneController) getSharedTaskDynamicZonesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.SharedTaskDynamicZone{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
