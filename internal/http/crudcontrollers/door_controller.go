...
func (e *DoorController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "doors/count", e.getDoorsCount, nil),
...
func (e *DoorController) createDoor(c echo.Context) error {
	...
	db := e.db.Get(models.Door{}, c).Model(&models.Door{})
...
// @Success 200 {array} models.Door
// @Failure 500 {string} string "Bad query request"
// @Router /doors/count [get]
func (e *DoorController) getDoorsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Door{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
