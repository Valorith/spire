...
func (e *PetController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "pets/count", e.getPetsCount, nil),
...
func (e *PetController) updatePet(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Pet] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *PetController) createPet(c echo.Context) error {
	...
	db := e.db.Get(models.Pet{}, c).Model(&models.Pet{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Pet{}, pet)
		...
		// record event
		event := fmt.Sprintf("Created [Pet] [%v] data [%v]", pet.ID, strings.Join(fields, ", "))
...
func (e *PetController) deletePet(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Pet] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
func (e *PetController) getPetsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Pet{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
