...
func (e *CharCreateCombinationController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "char_create_combinations/count", e.getCharCreateCombinationsCount, nil),
...
func (e *CharCreateCombinationController) updateCharCreateCombination(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharCreateCombination] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharCreateCombinationController) createCharCreateCombination(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharCreateCombination{}, charCreateCombination)
		...
		// record event
		event := fmt.Sprintf("Created [CharCreateCombination] [%v] data [%v]", charCreateCombination.Race, strings.Join(fields, ", "))
...
func (e *CharCreateCombinationController) deleteCharCreateCombination(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharCreateCombination] [%v] keys [%v]", result.Race, strings.Join(ids, ", "))
...
// getCharCreateCombinationsCount godoc
// @Id getCharCreateCombinationsCount
// @Summary Counts CharCreateCombinations
// @Accept json
// @Produce json
// @Tags CharCreateCombination
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharCreateCombination
// @Failure 500 {string} string "Bad query request"
// @Router /char_create_combinations/count [get]
func (e *CharCreateCombinationController) getCharCreateCombinationsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharCreateCombination{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
