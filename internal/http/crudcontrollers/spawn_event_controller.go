...
func (e *SpawnEventController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "spawn_events/count", e.getSpawnEventsCount, nil),
...
func (e *SpawnEventController) updateSpawnEvent(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [SpawnEvent] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *SpawnEventController) createSpawnEvent(c echo.Context) error {
	...
	db := e.db.Get(models.SpawnEvent{}, c).Model(&models.SpawnEvent{})
	...
	err := db.Create(&spawnEvent).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.SpawnEvent{}, spawnEvent)
		...
		// record event
		event := fmt.Sprintf("Created [SpawnEvent] [%v] data [%v]", spawnEvent.ID, strings.Join(fields, ", "))
...
func (e *SpawnEventController) deleteSpawnEvent(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [SpawnEvent] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getSpawnEventsCount godoc
// @Id getSpawnEventsCount
// @Summary Counts SpawnEvents
// @Accept json
// @Produce json
// @Tags SpawnEvent
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.SpawnEvent
// @Failure 500 {string} string "Bad query request"
// @Router /spawn_events/count [get]
func (e *SpawnEventController) getSpawnEventsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.SpawnEvent{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
