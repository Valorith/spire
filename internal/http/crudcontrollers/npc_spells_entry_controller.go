...
func (e *NpcSpellsEntryController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "npc_spells_entries/count", e.getNpcSpellsEntriesCount, nil),
...
func (e *NpcSpellsEntryController) createNpcSpellsEntry(c echo.Context) error {
	...
	db := e.db.Get(models.NpcSpellsEntry{}, c).Model(&models.NpcSpellsEntry{})
	...
	err := db.Create(&npcSpellsEntry).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getNpcSpellsEntriesCount godoc
// @Id getNpcSpellsEntriesCount
// @Summary Counts NpcSpellsEntries
// @Accept json
// @Produce json
// @Tags NpcSpellsEntry
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.NpcSpellsEntry
// @Failure 500 {string} string "Bad query request"
// @Router /npc_spells_entries/count [get]
func (e *NpcSpellsEntryController) getNpcSpellsEntriesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.NpcSpellsEntry{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
