...
func (e *GuildController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "guilds/count", e.getGuildsCount, nil),
...
func (e *GuildController) updateGuild(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Guild] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *GuildController) createGuild(c echo.Context) error {
	...
	db := e.db.Get(models.Guild{}, c).Model(&models.Guild{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Guild{}, guild)
		...
		// record event
		event := fmt.Sprintf("Created [Guild] [%v] data [%v]", guild.ID, strings.Join(fields, ", "))
...
func (e *GuildController) deleteGuild(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Guild] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getGuildsCount godoc
// @Id getGuildsCount
// @Summary Counts Guilds
// @Accept json
// @Produce json
// @Tags Guild
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Guild
// @Failure 500 {string} string "Bad query request"
// @Router /guilds/count [get]
func (e *GuildController) getGuildsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Guild{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
