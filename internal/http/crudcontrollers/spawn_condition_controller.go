...
func (e *SpawnConditionController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "spawn_conditions/count", e.getSpawnConditionsCount, nil),
...
func (e *SpawnConditionController) updateSpawnCondition(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [SpawnCondition] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *SpawnConditionController) createSpawnCondition(c echo.Context) error {
	...
	db := e.db.Get(models.SpawnCondition{}, c).Model(&models.SpawnCondition{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.SpawnCondition{}, spawnCondition)
		...
		// record event
		event := fmt.Sprintf("Created [SpawnCondition] [%v] data [%v]", spawnCondition.ID, strings.Join(fields, ", "))
...
func (e *SpawnConditionController) deleteSpawnCondition(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [SpawnCondition] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getSpawnConditionsCount godoc
// @Id getSpawnConditionsCount
// @Summary Counts SpawnConditions
// @Accept json
// @Produce json
// @Tags SpawnCondition
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.SpawnCondition
// @Failure 500 {string} string "Bad query request"
// @Router /spawn_conditions/count [get]
func (e *SpawnConditionController) getSpawnConditionsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.SpawnCondition{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
