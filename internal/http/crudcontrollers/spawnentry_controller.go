...
func (e *SpawnentryController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "spawnentries/count", e.getSpawnentriesCount, nil),
...
func (e *SpawnentryController) createSpawnentry(c echo.Context) error {
	...
	db := e.db.Get(models.Spawnentry{}, c).Model(&models.Spawnentry{})
	...
	err := db.Create(&spawnentry).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getSpawnentriesCount godoc
// @Id getSpawnentriesCount
// @Summary Counts Spawnentries
// @Accept json
// @Produce json
// @Tags Spawnentry
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Spawnentry
// @Failure 500 {string} string "Bad query request"
// @Router /spawnentries/count [get]
func (e *SpawnentryController) getSpawnentriesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Spawnentry{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
