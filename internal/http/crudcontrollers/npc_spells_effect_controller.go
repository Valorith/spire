...
func (e *NpcSpellsEffectController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "npc_spells_effects/count", e.getNpcSpellsEffectsCount, nil),
...
func (e *NpcSpellsEffectController) updateNpcSpellsEffect(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [NpcSpellsEffect] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *NpcSpellsEffectController) createNpcSpellsEffect(c echo.Context) error {
	...
	db := e.db.Get(models.NpcSpellsEffect{}, c).Model(&models.NpcSpellsEffect{})
	...
	err := db.Create(&npcSpellsEffect).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.NpcSpellsEffect{}, npcSpellsEffect)
		...
		// record event
		event := fmt.Sprintf("Created [NpcSpellsEffect] [%v] data [%v]", npcSpellsEffect.ID, strings.Join(fields, ", "))
...
func (e *NpcSpellsEffectController) deleteNpcSpellsEffect(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [NpcSpellsEffect] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getNpcSpellsEffectsCount godoc
// @Id getNpcSpellsEffectsCount
// @Summary Counts NpcSpellsEffects
// @Accept json
// @Produce json
// @Tags NpcSpellsEffect
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.NpcSpellsEffect
// @Failure 500 {string} string "Bad query request"
// @Router /npc_spells_effects/count [get]
func (e *NpcSpellsEffectController) getNpcSpellsEffectsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.NpcSpellsEffect{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
