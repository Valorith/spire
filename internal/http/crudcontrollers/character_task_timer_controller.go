...
func (e *CharacterTaskTimerController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_task_timers/count", e.getCharacterTaskTimersCount, nil),
...
func (e *CharacterTaskTimerController) updateCharacterTaskTimer(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterTaskTimer] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterTaskTimerController) createCharacterTaskTimer(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterTaskTimer{}, c).Model(&models.CharacterTaskTimer{})
	...
	err := db.Create(&characterTaskTimer).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterTaskTimer{}, characterTaskTimer)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterTaskTimer] [%v] data [%v]", characterTaskTimer.ID, strings.Join(fields, ", "))
...
func (e *CharacterTaskTimerController) deleteCharacterTaskTimer(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterTaskTimer] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getCharacterTaskTimersCount godoc
// @Id getCharacterTaskTimersCount
// @Summary Counts CharacterTaskTimers
// @Accept json
// @Produce json
// @Tags CharacterTaskTimer
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterTaskTimer
// @Failure 500 {string} string "Bad query request"
// @Router /character_task_timers/count [get]
func (e *CharacterTaskTimerController) getCharacterTaskTimersCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterTaskTimer{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
