...
func (e *DynamicZoneMemberController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "dynamic_zone_members/count", e.getDynamicZoneMembersCount, nil),
...
func (e *DynamicZoneMemberController) createDynamicZoneMember(c echo.Context) error {
	...
	db := e.db.Get(models.DynamicZoneMember{}, c).Model(&models.DynamicZoneMember{})
	...
	err := db.Create(&dynamicZoneMember).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getDynamicZoneMembersCount godoc
// @Id getDynamicZoneMembersCount
// @Summary Counts DynamicZoneMembers
// @Accept json
// @Produce json
// @Tags DynamicZoneMember
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.DynamicZoneMember
// @Failure 500 {string} string "Bad query request"
// @Router /dynamic_zone_members/count [get]
func (e *DynamicZoneMemberController) getDynamicZoneMembersCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.DynamicZoneMember{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
