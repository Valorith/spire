...
func (e *CharacterBandolierController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_bandoliers/count", e.getCharacterBandoliersCount, nil),
...
func (e *CharacterBandolierController) updateCharacterBandolier(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterBandolier] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterBandolierController) createCharacterBandolier(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterBandolier{}, c).Model(&models.CharacterBandolier{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterBandolier{}, characterBandolier)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterBandolier] [%v] data [%v]", characterBandolier.ID, strings.Join(fields, ", "))
...
func (e *CharacterBandolierController) deleteCharacterBandolier(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterBandolier] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getCharacterBandoliersCount godoc
// @Id getCharacterBandoliersCount
// @Summary Counts CharacterBandoliers
// @Accept json
// @Produce json
// @Tags CharacterBandolier
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterBandolier
// @Failure 500 {string} string "Bad query request"
// @Router /character_bandoliers/count [get]
func (e *CharacterBandolierController) getCharacterBandoliersCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterBandolier{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
