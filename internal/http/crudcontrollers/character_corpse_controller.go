...
func (e *CharacterCorpseController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_corpses/count", e.getCharacterCorpsesCount, nil),
...
func (e *CharacterCorpseController) updateCharacterCorpse(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterCorpse] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterCorpseController) createCharacterCorpse(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterCorpse{}, c).Model(&models.CharacterCorpse{})
	...
	err := db.Create(&characterCorpse).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterCorpse{}, characterCorpse)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterCorpse] [%v] data [%v]", characterCorpse.ID, strings.Join(fields, ", "))
...
func (e *CharacterCorpseController) deleteCharacterCorpse(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterCorpse] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getCharacterCorpsesCount godoc
// @Id getCharacterCorpsesCount
// @Summary Counts CharacterCorpses
// @Accept json
// @Produce json
// @Tags CharacterCorpse
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterCorpse
// @Failure 500 {string} string "Bad query request"
// @Router /character_corpses/count [get]
func (e *CharacterCorpseController) getCharacterCorpsesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterCorpse{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
