...
func (e *DataBucketController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "data_buckets/count", e.getDataBucketsCount, nil),
...
func (e *DataBucketController) updateDataBucket(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [DataBucket] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *DataBucketController) createDataBucket(c echo.Context) error {
	...
	db := e.db.Get(models.DataBucket{}, c).Model(&models.DataBucket{})
	...
	err := db.Create(&dataBucket).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.DataBucket{}, dataBucket)
		...
		// record event
		event := fmt.Sprintf("Created [DataBucket] [%v] data [%v]", dataBucket.ID, strings.Join(fields, ", "))
...
func (e *DataBucketController) deleteDataBucket(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [DataBucket] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getDataBucketsCount godoc
// @Id getDataBucketsCount
// @Summary Counts DataBuckets
// @Accept json
// @Produce json
// @Tags DataBucket
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.DataBucket
// @Failure 500 {string} string "Bad query request"
// @Router /data_buckets/count [get]
func (e *DataBucketController) getDataBucketsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.DataBucket{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
