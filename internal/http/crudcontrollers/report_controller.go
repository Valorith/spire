...
func (e *ReportController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "reports/count", e.getReportsCount, nil),
...
func (e *ReportController) updateReport(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Report] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *ReportController) createReport(c echo.Context) error {
	...
	db := e.db.Get(models.Report{}, c).Model(&models.Report{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Report{}, report)
		...
		// record event
		event := fmt.Sprintf("Created [Report] [%v] data [%v]", report.ID, strings.Join(fields, ", "))
...
func (e *ReportController) deleteReport(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Report] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// @Success 200 {array} models.Report
// @Failure 500 {string} string "Bad query request"
// @Router /reports/count [get]
func (e *ReportController) getReportsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Report{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
