...
func (e *AaRankPrereqController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "aa_rank_prereqs/count", e.getAaRankPrereqsCount, nil),
...
func (e *AaRankPrereqController) updateAaRankPrereq(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [AaRankPrereq] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *AaRankPrereqController) createAaRankPrereq(c echo.Context) error {
	...
	db := e.db.Get(models.AaRankPrereq{}, c).Model(&models.AaRankPrereq{})
	...
	err := db.Create(&aaRankPrereq).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.AaRankPrereq{}, aaRankPrereq)
		...
		// record event
		event := fmt.Sprintf("Created [AaRankPrereq] [%v] data [%v]", aaRankPrereq.RankId, strings.Join(fields, ", "))
...
func (e *AaRankPrereqController) deleteAaRankPrereq(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [AaRankPrereq] [%v] keys [%v]", result.RankId, strings.Join(ids, ", "))
...
// getAaRankPrereqsCount godoc
// @Id getAaRankPrereqsCount
// @Summary Counts AaRankPrereqs
// @Accept json
// @Produce json
// @Tags AaRankPrereq
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.AaRankPrereq
// @Failure 500 {string} string "Bad query request"
// @Router /aa_rank_prereqs/count [get]
func (e *AaRankPrereqController) getAaRankPrereqsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.AaRankPrereq{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
