...
func (e *CharacterTaskController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_tasks/count", e.getCharacterTasksCount, nil),
...
func (e *CharacterTaskController) updateCharacterTask(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterTask] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterTaskController) createCharacterTask(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterTask{}, c).Model(&models.CharacterTask{})
	...
	err := db.Create(&characterTask).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterTask{}, characterTask)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterTask] [%v] data [%v]", characterTask.Charid, strings.Join(fields, ", "))
...
func (e *CharacterTaskController) deleteCharacterTask(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterTask] [%v] keys [%v]", result.Charid, strings.Join(ids, ", "))
...
// getCharacterTasksCount godoc
// @Id getCharacterTasksCount
// @Summary Counts CharacterTasks
// @Accept json
// @Produce json
// @Tags CharacterTask
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterTask
// @Failure 500 {string} string "Bad query request"
// @Router /character_tasks/count [get]
func (e *CharacterTaskController) getCharacterTasksCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterTask{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
