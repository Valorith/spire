...
func (e *HorseController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "horses/count", e.getHorsesCount, nil),
...
func (e *HorseController) updateHorse(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Horse] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *HorseController) createHorse(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Horse{}, horse)
		...
		// record event
		event := fmt.Sprintf("Created [Horse] [%v] data [%v]", horse.ID, strings.Join(fields, ", "))
...
func (e *HorseController) deleteHorse(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Horse] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getHorsesCount godoc
// @Id getHorsesCount
// @Summary Counts Horses
// @Accept json
// @Produce json
// @Tags Horse
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Horse
// @Failure 500 {string} string "Bad query request"
// @Router /horses/count [get]
func (e *HorseController) getHorsesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Horse{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
