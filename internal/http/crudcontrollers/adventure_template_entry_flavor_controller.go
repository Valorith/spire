...
func (e *AdventureTemplateEntryFlavorController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "adventure_template_entry_flavors/count", e.getAdventureTemplateEntryFlavorsCount, nil),
...
func (e *AdventureTemplateEntryFlavorController) updateAdventureTemplateEntryFlavor(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [AdventureTemplateEntryFlavor] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *AdventureTemplateEntryFlavorController) createAdventureTemplateEntryFlavor(c echo.Context) error {
	...
	db := e.db.Get(models.AdventureTemplateEntryFlavor{}, c).Model(&models.AdventureTemplateEntryFlavor{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.AdventureTemplateEntryFlavor{}, adventureTemplateEntryFlavor)
		...
		// record event
		event := fmt.Sprintf("Created [AdventureTemplateEntryFlavor] [%v] data [%v]", adventureTemplateEntryFlavor.ID, strings.Join(fields, ", "))
...
func (e *AdventureTemplateEntryFlavorController) deleteAdventureTemplateEntryFlavor(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [AdventureTemplateEntryFlavor] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getAdventureTemplateEntryFlavorsCount godoc
// @Id getAdventureTemplateEntryFlavorsCount
// @Summary Counts AdventureTemplateEntryFlavors
// @Accept json
// @Produce json
// @Tags AdventureTemplateEntryFlavor
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.AdventureTemplateEntryFlavor
// @Failure 500 {string} string "Bad query request"
// @Router /adventure_template_entry_flavors/count [get]
func (e *AdventureTemplateEntryFlavorController) getAdventureTemplateEntryFlavorsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.AdventureTemplateEntryFlavor{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
