...
func (e *PlayerTitlesetController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "player_titlesets/count", e.getPlayerTitlesetsCount, nil),
...
func (e *PlayerTitlesetController) createPlayerTitleset(c echo.Context) error {
	...
	db := e.db.Get(models.PlayerTitleset{}, c).Model(&models.PlayerTitleset{})
	...
	err := db.Create(&playerTitleset).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getPlayerTitlesetsCount godoc
// @Id getPlayerTitlesetsCount
// @Summary Counts PlayerTitlesets
// @Accept json
// @Produce json
// @Tags PlayerTitleset
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.PlayerTitleset
// @Failure 500 {string} string "Bad query request"
// @Router /player_titlesets/count [get]
func (e *PlayerTitlesetController) getPlayerTitlesetsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.PlayerTitleset{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
