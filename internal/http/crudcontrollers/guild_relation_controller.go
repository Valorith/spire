...
func (e *GuildRelationController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "guild_relations/count", e.getGuildRelationsCount, nil),
...
func (e *GuildRelationController) createGuildRelation(c echo.Context) error {
	...
	db := e.db.Get(models.GuildRelation{}, c).Model(&models.GuildRelation{})
	...
	err := db.Create(&guildRelation).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getGuildRelationsCount godoc
// @Id getGuildRelationsCount
// @Summary Counts GuildRelations
// @Accept json
// @Produce json
// @Tags GuildRelation
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.GuildRelation
// @Failure 500 {string} string "Bad query request"
// @Router /guild_relations/count [get]
func (e *GuildRelationController) getGuildRelationsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.GuildRelation{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
