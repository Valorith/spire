...
func (e *QuestGlobalController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "quest_globals/count", e.getQuestGlobalsCount, nil),
...
func (e *QuestGlobalController) createQuestGlobal(c echo.Context) error {
	...
	db := e.db.Get(models.QuestGlobal{}, c).Model(&models.QuestGlobal{})
	...
	err := db.Create(&questGlobal).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getQuestGlobalsCount godoc
// @Id getQuestGlobalsCount
// @Summary Counts QuestGlobals
// @Accept json
// @Produce json
// @Tags QuestGlobal
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.QuestGlobal
// @Failure 500 {string} string "Bad query request"
// @Router /quest_globals/count [get]
func (e *QuestGlobalController) getQuestGlobalsCount(c echo.Context) error {
	var count int64
