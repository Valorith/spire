...
func (e *ObjectController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "objects/count", e.getObjectsCount, nil),
...
func (e *ObjectController) updateObject(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Object] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *ObjectController) createObject(c echo.Context) error {
	...
	db := e.db.Get(models.Object{}, c).Model(&models.Object{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Object{}, object)
		...
		// record event
		event := fmt.Sprintf("Created [Object] [%v] data [%v]", object.ID, strings.Join(fields, ", "))
...
func (e *ObjectController) deleteObject(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Object] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// @Success 200 {array} models.Object
// @Failure 500 {string} string "Bad query request"
// @Router /objects/count [get]
func (e *ObjectController) getObjectsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Object{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
