...
func (e *ObjectContentController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "object_contents/count", e.getObjectContentsCount, nil),
...
func (e *ObjectContentController) updateObjectContent(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [ObjectContent] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *ObjectContentController) createObjectContent(c echo.Context) error {
	...
	db := e.db.Get(models.ObjectContent{}, c).Model(&models.ObjectContent{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.ObjectContent{}, objectContent)
		...
		// record event
		event := fmt.Sprintf("Created [ObjectContent] [%v] data [%v]", objectContent.Parentid, strings.Join(fields, ", "))
...
func (e *ObjectContentController) deleteObjectContent(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [ObjectContent] [%v] keys [%v]", result.Parentid, strings.Join(ids, ", "))
...
// getObjectContentsCount godoc
// @Id getObjectContentsCount
// @Summary Counts ObjectContents
// @Accept json
// @Produce json
// @Tags ObjectContent
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.ObjectContent
// @Failure 500 {string} string "Bad query request"
// @Router /object_contents/count [get]
func (e *ObjectContentController) getObjectContentsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.ObjectContent{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
