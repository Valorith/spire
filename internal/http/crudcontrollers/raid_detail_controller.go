...
func (e *RaidDetailController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "raid_details/count", e.getRaidDetailsCount, nil),
...
func (e *RaidDetailController) createRaidDetail(c echo.Context) error {
	...
	db := e.db.Get(models.RaidDetail{}, c).Model(&models.RaidDetail{})
	...
	err := db.Create(&raidDetail).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getRaidDetailsCount godoc
// @Id getRaidDetailsCount
// @Summary Counts RaidDetails
// @Accept json
// @Produce json
// @Tags RaidDetail
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.RaidDetail
// @Failure 500 {string} string "Bad query request"
// @Router /raid_details/count [get]
func (e *RaidDetailController) getRaidDetailsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.RaidDetail{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
