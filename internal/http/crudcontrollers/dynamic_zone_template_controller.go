...
func (e *DynamicZoneTemplateController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "dynamic_zone_templates/count", e.getDynamicZoneTemplatesCount, nil),
...
func (e *DynamicZoneTemplateController) updateDynamicZoneTemplate(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [DynamicZoneTemplate] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *DynamicZoneTemplateController) createDynamicZoneTemplate(c echo.Context) error {
	...
	db := e.db.Get(models.DynamicZoneTemplate{}, c).Model(&models.DynamicZoneTemplate{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.DynamicZoneTemplate{}, dynamicZoneTemplate)
		...
		// record event
		event := fmt.Sprintf("Created [DynamicZoneTemplate] [%v] data [%v]", dynamicZoneTemplate.ID, strings.Join(fields, ", "))
...
func (e *DynamicZoneTemplateController) deleteDynamicZoneTemplate(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [DynamicZoneTemplate] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getDynamicZoneTemplatesCount godoc
// @Id getDynamicZoneTemplatesCount
// @Summary Counts DynamicZoneTemplates
// @Accept json
// @Produce json
// @Tags DynamicZoneTemplate
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.DynamicZoneTemplate
// @Failure 500 {string} string "Bad query request"
// @Router /dynamic_zone_templates/count [get]
func (e *DynamicZoneTemplateController) getDynamicZoneTemplatesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.DynamicZoneTemplate{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
