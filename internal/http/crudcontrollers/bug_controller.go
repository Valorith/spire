...
func (e *BugController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "bugs/count", e.getBugsCount, nil),
...
func (e *BugController) updateBug(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Bug] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *BugController) createBug(c echo.Context) error {
	...
	db := e.db.Get(models.Bug{}, c).Model(&models.Bug{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Bug{}, bug)
		...
		// record event
		event := fmt.Sprintf("Created [Bug] [%v] data [%v]", bug.ID, strings.Join(fields, ", "))
...
func (e *BugController) deleteBug(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Bug] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// @Success 200 {array} models.Bug
// @Failure 500 {string} string "Bad query request"
// @Router /bugs/count [get]
func (e *BugController) getBugsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Bug{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
