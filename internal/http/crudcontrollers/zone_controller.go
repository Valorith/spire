...
func (e *ZoneController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "zones/count", e.getZonesCount, nil),
...
func (e *ZoneController) updateZone(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Zone] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *ZoneController) createZone(c echo.Context) error {
	...
	db := e.db.Get(models.Zone{}, c).Model(&models.Zone{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Zone{}, zone)
		...
		// record event
		event := fmt.Sprintf("Created [Zone] [%v] data [%v]", zone.ID, strings.Join(fields, ", "))
...
func (e *ZoneController) deleteZone(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Zone] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// @Success 200 {array} models.Zone
// @Failure 500 {string} string "Bad query request"
// @Router /zones/count [get]
func (e *ZoneController) getZonesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Zone{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
