...
func (e *BookController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "books/count", e.getBooksCount, nil),
...
func (e *BookController) updateBook(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Book] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *BookController) createBook(c echo.Context) error {
	...
	db := e.db.Get(models.Book{}, c).Model(&models.Book{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Book{}, book)
		...
		// record event
		event := fmt.Sprintf("Created [Book] [%v] data [%v]", book.ID, strings.Join(fields, ", "))
...
func (e *BookController) deleteBook(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Book] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
func (e *BookController) getBooksCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Book{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
