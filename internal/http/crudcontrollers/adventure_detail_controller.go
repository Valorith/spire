...
func (e *AdventureDetailController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "adventure_details/count", e.getAdventureDetailsCount, nil),
...
func (e *AdventureDetailController) updateAdventureDetail(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [AdventureDetail] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *AdventureDetailController) createAdventureDetail(c echo.Context) error {
	...
	db := e.db.Get(models.AdventureDetail{}, c).Model(&models.AdventureDetail{})
	...
	err := db.Create(&adventureDetail).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.AdventureDetail{}, adventureDetail)
		...
		// record event
		event := fmt.Sprintf("Created [AdventureDetail] [%v] data [%v]", adventureDetail.ID, strings.Join(fields, ", "))
...
func (e *AdventureDetailController) deleteAdventureDetail(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [AdventureDetail] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getAdventureDetailsCount godoc
// @Id getAdventureDetailsCount
// @Summary Counts AdventureDetails
// @Accept json
// @Produce json
// @Tags AdventureDetail
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.AdventureDetail
// @Failure 500 {string} string "Bad query request"
// @Router /adventure_details/count [get]
func (e *AdventureDetailController) getAdventureDetailsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.AdventureDetail{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
