...
func (e *CharacterBuffController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "character_buffs/count", e.getCharacterBuffsCount, nil),
...
func (e *CharacterBuffController) updateCharacterBuff(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CharacterBuff] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CharacterBuffController) createCharacterBuff(c echo.Context) error {
	...
	db := e.db.Get(models.CharacterBuff{}, c).Model(&models.CharacterBuff{})
	...
	err := db.Create(&characterBuff).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CharacterBuff{}, characterBuff)
		...
		// record event
		event := fmt.Sprintf("Created [CharacterBuff] [%v] data [%v]", characterBuff.CharacterId, strings.Join(fields, ", "))
...
func (e *CharacterBuffController) deleteCharacterBuff(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CharacterBuff] [%v] keys [%v]", result.CharacterId, strings.Join(ids, ", "))
...
// getCharacterBuffsCount godoc
// @Id getCharacterBuffsCount
// @Summary Counts CharacterBuffs
// @Accept json
// @Produce json
// @Tags CharacterBuff
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CharacterBuff
// @Failure 500 {string} string "Bad query request"
// @Router /character_buffs/count [get]
func (e *CharacterBuffController) getCharacterBuffsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CharacterBuff{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
