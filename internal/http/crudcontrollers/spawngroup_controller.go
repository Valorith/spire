...
func (e *SpawngroupController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "spawngroups/count", e.getSpawngroupsCount, nil),
...
func (e *SpawngroupController) updateSpawngroup(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Spawngroup] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *SpawngroupController) createSpawngroup(c echo.Context) error {
	...
	db := e.db.Get(models.Spawngroup{}, c).Model(&models.Spawngroup{})
	...
	err := db.Create(&spawngroup).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Spawngroup{}, spawngroup)
		...
		// record event
		event := fmt.Sprintf("Created [Spawngroup] [%v] data [%v]", spawngroup.ID, strings.Join(fields, ", "))
...
func (e *SpawngroupController) deleteSpawngroup(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Spawngroup] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getSpawngroupsCount godoc
// @Id getSpawngroupsCount
// @Summary Counts Spawngroups
// @Accept json
// @Produce json
// @Tags Spawngroup
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Spawngroup
// @Failure 500 {string} string "Bad query request"
// @Router /spawngroups/count [get]
func (e *SpawngroupController) getSpawngroupsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Spawngroup{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
