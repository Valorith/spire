...
func (e *DiscordWebhookController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "discord_webhooks/count", e.getDiscordWebhooksCount, nil),
...
func (e *DiscordWebhookController) createDiscordWebhook(c echo.Context) error {
	...
	db := e.db.Get(models.DiscordWebhook{}, c).Model(&models.DiscordWebhook{})
	...
	err := db.Create(&discordWebhook).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getDiscordWebhooksCount godoc
// @Id getDiscordWebhooksCount
// @Summary Counts DiscordWebhooks
// @Accept json
// @Produce json
// @Tags DiscordWebhook
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.DiscordWebhook
// @Failure 500 {string} string "Bad query request"
// @Router /discord_webhooks/count [get]
func (e *DiscordWebhookController) getDiscordWebhooksCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.DiscordWebhook{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
