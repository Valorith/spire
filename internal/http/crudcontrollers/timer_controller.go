...
func (e *TimerController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "timers/count", e.getTimersCount, nil),
...
func (e *TimerController) getTimer(c echo.Context) error {
	...
	// key param [type] position [2] type [mediumint]
...
func (e *TimerController) createTimer(c echo.Context) error {
	...
	db := e.db.Get(models.Timer{}, c).Model(&models.Timer{})
...
// getTimersCount godoc
// @Id getTimersCount
// @Summary Counts Timers
// @Accept json
// @Produce json
// @Tags Timer
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.Timer
// @Failure 500 {string} string "Bad query request"
// @Router /timers/count [get]
func (e *TimerController) getTimersCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Timer{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
