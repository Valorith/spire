...
func (e *CompletedTaskController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "completed_tasks/count", e.getCompletedTasksCount, nil),
...
func (e *CompletedTaskController) updateCompletedTask(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CompletedTask] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CompletedTaskController) createCompletedTask(c echo.Context) error {
	...
	db := e.db.Get(models.CompletedTask{}, c).Model(&models.CompletedTask{})
	...
	err := db.Create(&completedTask).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CompletedTask{}, completedTask)
		...
		// record event
		event := fmt.Sprintf("Created [CompletedTask] [%v] data [%v]", completedTask.Charid, strings.Join(fields, ", "))
...
func (e *CompletedTaskController) deleteCompletedTask(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CompletedTask] [%v] keys [%v]", result.Charid, strings.Join(ids, ", "))
...
// getCompletedTasksCount godoc
// @Id getCompletedTasksCount
// @Summary Counts CompletedTasks
// @Accept json
// @Produce json
// @Tags CompletedTask
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CompletedTask
// @Failure 500 {string} string "Bad query request"
// @Router /completed_tasks/count [get]
func (e *CompletedTaskController) getCompletedTasksCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CompletedTask{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
