...
func (e *FactionListController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "faction_lists/count", e.getFactionListsCount, nil),
...
func (e *FactionListController) updateFactionList(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [FactionList] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *FactionListController) createFactionList(c echo.Context) error {
	...
	db := e.db.Get(models.FactionList{}, c).Model(&models.FactionList{})
	...
	err := db.Create(&factionList).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.FactionList{}, factionList)
		...
		// record event
		event := fmt.Sprintf("Created [FactionList] [%v] data [%v]", factionList.ID, strings.Join(fields, ", "))
...
func (e *FactionListController) deleteFactionList(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [FactionList] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getFactionListsCount godoc
// @Id getFactionListsCount
// @Summary Counts FactionLists
// @Accept json
// @Produce json
// @Tags FactionList
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.FactionList
// @Failure 500 {string} string "Bad query request"
// @Router /faction_lists/count [get]
func (e *FactionListController) getFactionListsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.FactionList{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
