...
func (e *LoginApiTokenController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "login_api_tokens/count", e.getLoginApiTokensCount, nil),
...
func (e *LoginApiTokenController) createLoginApiToken(c echo.Context) error {
	...
	db := e.db.Get(models.LoginApiToken{}, c).Model(&models.LoginApiToken{})
	...
	err := db.Create(&loginApiToken).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
...
// getLoginApiTokensCount godoc
// @Id getLoginApiTokensCount
// @Summary Counts LoginApiTokens
// @Accept json
// @Produce json
// @Tags LoginApiToken
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.LoginApiToken
// @Failure 500 {string} string "Bad query request"
// @Router /login_api_tokens/count [get]
func (e *LoginApiTokenController) getLoginApiTokensCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.LoginApiToken{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
