...
func (e *TrapController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "traps/count", e.getTrapsCount, nil),
...
func (e *TrapController) updateTrap(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [Trap] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *TrapController) createTrap(c echo.Context) error {
	...
	db := e.db.Get(models.Trap{}, c).Model(&models.Trap{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.Trap{}, trap)
		...
		// record event
		event := fmt.Sprintf("Created [Trap] [%v] data [%v]", trap.ID, strings.Join(fields, ", "))
...
func (e *TrapController) deleteTrap(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [Trap] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// @Success 200 {array} models.Trap
// @Failure 500 {string} string "Bad query request"
// @Router /traps/count [get]
func (e *TrapController) getTrapsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Trap{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
