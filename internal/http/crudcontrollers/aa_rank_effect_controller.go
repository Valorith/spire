...
func (e *AaRankEffectController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "aa_rank_effects/count", e.getAaRankEffectsCount, nil),
...
func (e *AaRankEffectController) createAaRankEffect(c echo.Context) error {
	...
	db := e.db.Get(models.AaRankEffect{}, c).Model(&models.AaRankEffect{})
...
// getAaRankEffectsCount godoc
// @Id getAaRankEffectsCount
// @Summary Counts AaRankEffects
// @Accept json
// @Produce json
// @Tags AaRankEffect
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.AaRankEffect
// @Failure 500 {string} string "Bad query request"
// @Router /aa_rank_effects/count [get]
func (e *AaRankEffectController) getAaRankEffectsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.AaRankEffect{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
