...
func (e *SharedTaskMemberController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "shared_task_members/count", e.getSharedTaskMembersCount, nil),
...
func (e *SharedTaskMemberController) updateSharedTaskMember(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [SharedTaskMember] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *SharedTaskMemberController) createSharedTaskMember(c echo.Context) error {
	...
	db := e.db.Get(models.SharedTaskMember{}, c).Model(&models.SharedTaskMember{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.SharedTaskMember{}, sharedTaskMember)
		...
		// record event
		event := fmt.Sprintf("Created [SharedTaskMember] [%v] data [%v]", sharedTaskMember.SharedTaskId, strings.Join(fields, ", "))
...
func (e *SharedTaskMemberController) deleteSharedTaskMember(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [SharedTaskMember] [%v] keys [%v]", result.SharedTaskId, strings.Join(ids, ", "))
...
// getSharedTaskMembersCount godoc
// @Id getSharedTaskMembersCount
// @Summary Counts SharedTaskMembers
// @Accept json
// @Produce json
// @Tags SharedTaskMember
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.SharedTaskMember
// @Failure 500 {string} string "Bad query request"
// @Router /shared_task_members/count [get]
func (e *SharedTaskMemberController) getSharedTaskMembersCount(c echo.Context) error {
	var count int64
