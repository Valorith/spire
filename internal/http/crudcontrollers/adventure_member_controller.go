...
func (e *AdventureMemberController) updateAdventureMember(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [AdventureMember] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *AdventureMemberController) createAdventureMember(c echo.Context) error {
	...
	db := e.db.Get(models.AdventureMember{}, c).Model(&models.AdventureMember{})
	...
	err := db.Create(&adventureMember).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.AdventureMember{}, adventureMember)
		...
		// record event
		event := fmt.Sprintf("Created [AdventureMember] [%v] data [%v]", adventureMember.Charid, strings.Join(fields, ", "))
...
func (e *AdventureMemberController) deleteAdventureMember(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [AdventureMember] [%v] keys [%v]", result.Charid, strings.Join(ids, ", "))
