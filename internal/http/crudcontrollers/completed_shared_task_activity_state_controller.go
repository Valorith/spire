...
func (e *CompletedSharedTaskActivityStateController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "completed_shared_task_activity_states/count", e.getCompletedSharedTaskActivityStatesCount, nil),
...
func (e *CompletedSharedTaskActivityStateController) updateCompletedSharedTaskActivityState(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [CompletedSharedTaskActivityState] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *CompletedSharedTaskActivityStateController) createCompletedSharedTaskActivityState(c echo.Context) error {
	...
	db := e.db.Get(models.CompletedSharedTaskActivityState{}, c).Model(&models.CompletedSharedTaskActivityState{})
	...
	err := db.Create(&completedSharedTaskActivityState).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.CompletedSharedTaskActivityState{}, completedSharedTaskActivityState)
		...
		// record event
		event := fmt.Sprintf("Created [CompletedSharedTaskActivityState] [%v] data [%v]", completedSharedTaskActivityState.SharedTaskId, strings.Join(fields, ", "))
...
func (e *CompletedSharedTaskActivityStateController) deleteCompletedSharedTaskActivityState(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [CompletedSharedTaskActivityState] [%v] keys [%v]", result.SharedTaskId, strings.Join(ids, ", "))
...
// getCompletedSharedTaskActivityStatesCount godoc
// @Id getCompletedSharedTaskActivityStatesCount
// @Summary Counts CompletedSharedTaskActivityStates
// @Accept json
// @Produce json
// @Tags CompletedSharedTaskActivityState
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.CompletedSharedTaskActivityState
// @Failure 500 {string} string "Bad query request"
// @Router /completed_shared_task_activity_states/count [get]
func (e *CompletedSharedTaskActivityStateController) getCompletedSharedTaskActivityStatesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.CompletedSharedTaskActivityState{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
