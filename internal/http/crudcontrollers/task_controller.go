...
func (e *TaskController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "tasks/count", e.getTasksCount, nil),
...
func (e *TaskController) createTask(c echo.Context) error {
	...
	db := e.db.Get(models.Task{}, c).Model(&models.Task{})
...
func (e *TaskController) getTasksCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Task{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
