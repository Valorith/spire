...
func (e *TaskActivityController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "task_activities/count", e.getTaskActivitiesCount, nil),
...
func (e *TaskActivityController) updateTaskActivity(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [TaskActivity] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *TaskActivityController) createTaskActivity(c echo.Context) error {
	...
	db := e.db.Get(models.TaskActivity{}, c).Model(&models.TaskActivity{})
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.TaskActivity{}, taskActivity)
		...
		// record event
		event := fmt.Sprintf("Created [TaskActivity] [%v] data [%v]", taskActivity.Taskid, strings.Join(fields, ", "))
...
func (e *TaskActivityController) deleteTaskActivity(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [TaskActivity] [%v] keys [%v]", result.Taskid, strings.Join(ids, ", "))
...
// getTaskActivitiesCount godoc
// @Id getTaskActivitiesCount
// @Summary Counts TaskActivities
// @Accept json
// @Produce json
// @Tags TaskActivity
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.TaskActivity
// @Failure 500 {string} string "Bad query request"
// @Router /task_activities/count [get]
func (e *TaskActivityController) getTaskActivitiesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.TaskActivity{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
