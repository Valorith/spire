...
func (e *ExpeditionLockoutController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "expedition_lockouts/count", e.getExpeditionLockoutsCount, nil),
...
func (e *ExpeditionLockoutController) updateExpeditionLockout(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [ExpeditionLockout] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *ExpeditionLockoutController) createExpeditionLockout(c echo.Context) error {
	...
	db := e.db.Get(models.ExpeditionLockout{}, c).Model(&models.ExpeditionLockout{})
	...
	err := db.Create(&expeditionLockout).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.ExpeditionLockout{}, expeditionLockout)
		...
		// record event
		event := fmt.Sprintf("Created [ExpeditionLockout] [%v] data [%v]", expeditionLockout.ID, strings.Join(fields, ", "))
...
func (e *ExpeditionLockoutController) deleteExpeditionLockout(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [ExpeditionLockout] [%v] keys [%v]", result.ID, strings.Join(ids, ", "))
...
// getExpeditionLockoutsCount godoc
// @Id getExpeditionLockoutsCount
// @Summary Counts ExpeditionLockouts
// @Accept json
// @Produce json
// @Tags ExpeditionLockout
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.ExpeditionLockout
// @Failure 500 {string} string "Bad query request"
// @Router /expedition_lockouts/count [get]
func (e *ExpeditionLockoutController) getExpeditionLockoutsCount(c echo.Context) error {
	var count int64
