...
func (e *LogsysCategoryController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "logsys_categories/count", e.getLogsysCategoriesCount, nil),
...
func (e *LogsysCategoryController) createLogsysCategory(c echo.Context) error {
	...
	db := e.db.Get(models.LogsysCategory{}, c).Model(&models.LogsysCategory{})
...
// getLogsysCategoriesCount godoc
// @Id getLogsysCategoriesCount
// @Summary Counts LogsysCategories
// @Accept json
// @Produce json
// @Tags LogsysCategory
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.LogsysCategory
// @Failure 500 {string} string "Bad query request"
// @Router /logsys_categories/count [get]
func (e *LogsysCategoryController) getLogsysCategoriesCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.LogsysCategory{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
