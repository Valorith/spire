...
func (e *IpExemptionController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "ip_exemptions/count", e.getIpExemptionsCount, nil),
...
func (e *IpExemptionController) updateIpExemption(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil && len(diff) > 0 {
		...
		// record event
		event := fmt.Sprintf("Updated [IpExemption] [%v] fields [%v]", strings.Join(ids, ", "), strings.Join(fieldsUpdated, ", "))
...
func (e *IpExemptionController) createIpExemption(c echo.Context) error {
	...
	db := e.db.Get(models.IpExemption{}, c).Model(&models.IpExemption{})
	...
	err := db.Create(&ipExemption).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
	...
	if e.db.GetSpireDb() != nil {
		...
		diff := database.ResultDifference(models.IpExemption{}, ipExemption)
		...
		// record event
		event := fmt.Sprintf("Created [IpExemption] [%v] data [%v]", ipExemption.ExemptionId, strings.Join(fields, ", "))
...
func (e *IpExemptionController) deleteIpExemption(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [IpExemption] [%v] keys [%v]", result.ExemptionId, strings.Join(ids, ", "))
...
// getIpExemptionsCount godoc
// @Id getIpExemptionsCount
// @Summary Counts IpExemptions
// @Accept json
// @Produce json
// @Tags IpExemption
// @Param includes query string false "Relationships [all] for all [number] for depth of relationships to load or [.] separated relationship names"
...
// @Success 200 {array} models.IpExemption
// @Failure 500 {string} string "Bad query request"
// @Router /ip_exemptions/count [get]
func (e *IpExemptionController) getIpExemptionsCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.IpExemption{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
