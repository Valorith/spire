...
export const ExportDialog = ({ open, setOpen }) => {
  ...
  const doExport = useCallback(async () => {
    ...
    const writeEqgMod = async (
      ...
      if (terrain) {
        const water_e = await fetch(staticAssetUrl('water_e.dds')).then((r) =>
          r.arrayBuffer()
        );
        textureNames.push("water_e.dds");
        writePfsFile("water_e.dds", new Uint8Array(water_e));
        const water_n = await fetch(staticAssetUrl('water_n.dds')).then((r) =>
          r.arrayBuffer()
        );
        ...
        const test = await fetch(staticAssetUrl('ra_watertest_c_01.dds')).then((r) =>
          r.arrayBuffer()
        );
